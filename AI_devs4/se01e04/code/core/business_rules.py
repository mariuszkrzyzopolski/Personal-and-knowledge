...
from core.constants import (
    ...
    CLOSED_ROUTE_DESTINATIONS,
    CLOSED_ROUTE_ALLOWED_CATEGORIES,
)
...
def calculate_fee(
    category: str, weight_kg: float, distance_km: float, regional_boundaries: int
) -> dict:
    base_fee = CATEGORY_BASE_FEES.get(category, 10)
...
def can_use_closed_route(category: str, destination: str) -> bool:
    if (
        destination in CLOSED_ROUTE_DESTINATIONS
        and category in CLOSED_ROUTE_ALLOWED_CATEGORIES
    ):
        return True
    return False
